...
class AudioProcessing(object):
	...
	def __init__(self, input_audio_path):
		...
		#(self.audio_data, self.sample_freq) = librosa.load(input_audio_path, dtype=np.float32)
		print (self.audio_data.shape[0])
		print(type(self.audio_data[0]))
		print('before nomalize:', np.max(self.audio_data), np.min(self.audio_data), np.mean(self.audio_data))

# 		self.audio_data = preprocessing.minmax_scale(self.audio_data, (-10000, 10000))
	...
	def set_echo(self, delay):
		...
		for count, e in enumerate(self.audio_data):
			output_audio[count] = e + int(0.9*self.audio_data[count - output_delay]) + int(0.81*self.audio_data[count - 2*output_delay]) + int(0.729*self.audio_data[count - 3*output_delay])
			#print ('count:',count,' e:',e)

		self.audio_data = output_audio
	def set_reverb(self, delay, decay):
		'''Applies an echo that is 0...<input audio duration in seconds> seconds from the beginning'''
		output_audio = np.zeros(len(self.audio_data))
		output_delay = delay * self.sample_freq

		for count, e in enumerate(self.audio_data):
			output_audio[count] = e + decay*self.audio_data[count - int(output_delay)]+ decay * decay * self.audio_data[count - int(output_delay)- int(output_delay)]
