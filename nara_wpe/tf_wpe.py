...
def get_power_inverse(signal):
    ...
    inverse_power = tf.math.reciprocal(tf.maximum(power, eps))
...
def online_wpe_step(
    ...
    window_conj = tf.math.conj(window)
    pred = (
        input_buffer[-1] -
        tf.einsum('fid,fi->fd', tf.math.conj(filter_taps), window)
    )
    ...
    filter_taps_k = (
        filter_taps +
        tf.einsum('fi,fm->fim', kalman_gain, tf.math.conj(pred))
    )
